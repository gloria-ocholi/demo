...
describe('Employee e2e test', () => {
  ...
  describe('new Employee page', () => {
    ...
    it('should create an instance of Employee', () => {
      cy.get(`[data-cy="firstName"]`).type('Felix');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Felix');

      cy.get(`[data-cy="lastName"]`).type('Bashirian');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Bashirian');

      cy.get(`[data-cy="email"]`).type('Hassie_Nader50@gmail.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Hassie_Nader50@gmail.com');

      cy.get(`[data-cy="phoneNumber"]`).type('simplistic octave avalanche');
      cy.get(`[data-cy="phoneNumber"]`).should('have.value', 'simplistic octave avalanche');

      cy.get(`[data-cy="hireDate"]`).type('2025-05-28T23:25');
      cy.get(`[data-cy="hireDate"]`).blur();
      cy.get(`[data-cy="hireDate"]`).should('have.value', '2025-05-28T23:25');

      cy.get(`[data-cy="salary"]`).type('21364');
      cy.get(`[data-cy="salary"]`).should('have.value', '21364');

      cy.get(`[data-cy="commissionPct"]`).type('1042');
      cy.get(`[data-cy="commissionPct"]`).should('have.value', '1042');
